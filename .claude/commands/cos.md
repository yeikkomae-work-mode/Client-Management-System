...
Read `.claude/agents/chief-of-staff.md` and operate under it for this whole request. That file
holds the routing table, the authority rules, the connector rule, the folder-separation rule,
and the session-memory protocol — **don't restate them here, follow them from there.**
...
3. Cite the source file for every item. If the rollup's last-touched dates are older than the
   ...
   dates as current.
4. Then ask what to work on. Don't pick for Eikko, and don't start work off the back of this.
...
3. **Check the authority table before delegating**, not after. If any step would end in a send,
