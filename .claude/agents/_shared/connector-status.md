...
**Every agent — the 10 specialists and the `chief-operating-officer` — reads this at runtime before claiming any tool is live.** Update this ONE file when you fix/authorize a connector — don't hand-edit the status inside individual agent files.
...
| Notion (native connector) | ✅ Connected | **Corrected 2026-08-25 — was documented wrong.** This row previously said "eikko mae ybanez's Space." Verified live via `notion-get-teams`: the native connector is actually scoped to **"Chris Drew's Space HQ"** (Satlas) — likely the same workspace as the "Satlas team Notion ('Cold Email' hub)" row below, not a personal/VA-Command-Center workspace. Every native Notion tool call in a session goes to **Satlas's workspace** until this is re-pointed. Eikko's personal Notion is a **separate, currently unconnected** workspace — he has a personal integration token for it, but no native connector; reconnecting one requires claude.ai connector settings (interactive, can't be done from a session). Don't write anything meant for the VA Command Center / personal task tracking through this connector until it's confirmed pointed at the right workspace. |
| Satlas team Notion ("Cold Email" hub) | 🟡 Needs reconciling | Documented as a separate not-connected workspace from "this workspace" above — but the native connector above turned out to *be* Chris Drew's Space HQ. May be the same workspace as the row above, not a second one. Needs Eikko to confirm before either row is trusted. |
| Notion (Eikko's personal workspace) | 🟡 Not connected via native tools | Eikko holds a personal internal-integration token (`ntn_...`) for this workspace, shared in chat 2026-08-25 — **not written to any file, session is ephemeral, treat the pasted token as needing rotation.** No native MCP connector reaches this workspace currently. To use it durably: either reconnect a native Notion connector to this workspace in claude.ai settings, or add the token to `RESOURCES/Tools & API Details/tools_api_details.md` (gitignored, same pattern as the raw Apollo/PlusVibe keys) **on Eikko's own machine** — this remote session's filesystem doesn't persist and can't write that file for him. |
